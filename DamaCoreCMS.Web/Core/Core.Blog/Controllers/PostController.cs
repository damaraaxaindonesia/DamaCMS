        ...
        public ActionResult PublishPost(long Id = 0)
        {
            if (Id > 0)
            {
                ...
                if (item.PostStatus != NccPost.NccPostStatus.Published)
                {
                    ...
                    ShowMessage("Post Published Successfully", DamaCoreCMS.Framework.Core.Mvc.Views.MessageType.Success, false, true);
                }
                else
                {
                    item.PostStatus = NccPost.NccPostStatus.Draft;
                    ShowMessage("Post Save as dtaft", DamaCoreCMS.Framework.Core.Mvc.Views.MessageType.Success, false, true);
                }
