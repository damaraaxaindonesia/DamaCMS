...
namespace Core.Admin.Models.ViewModels
{
    public class UserViewModel
    {
        ...
        private List<MenuItemViewModel> GetMenuItems(List<MenuItem> menuItems, string moduleName)
        {
            ...
            foreach (var item in menuItems)
            {
                ...
                if (_permission != null)
                {
                    var mip = _permission.PermissionDetails?.Where(x => x.ModuleName == moduleName && x.Action == action && x.Controller == controller).FirstOrDefault();
                    if (mip != null)
                    {
