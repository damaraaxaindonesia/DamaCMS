...
using DamaCoreCMS.Core.Modules.Setup.Models.ViewModels;
...
namespace DamaCoreCMS.Core.Modules.Setup.Controllers
{
    [AllowAnonymous]
            ...
            if (SetupHelper.IsDbCreateComplete == true && SetupHelper.IsAdminCreateComplete == false)
            {
                ...
                switch (supportedDatabases)
                {
                    case SupportedDatabases.MSSQL:
                        optionBuilder.UseSqlServer(SetupHelper.ConnectionString, opts => opts.MigrationsAssembly("DamaCoreCMS.Framework"));
                        break;
                    case SupportedDatabases.MsSqlLocalStorage:
                        break;
                    case SupportedDatabases.MySql:
                        optionBuilder.UseMySql(SetupHelper.ConnectionString, opts => opts.MigrationsAssembly("DamaCoreCMS.Framework"));
                        break;
                    case SupportedDatabases.SqLite:
                        optionBuilder.UseSqlite(SetupHelper.ConnectionString, opts => opts.MigrationsAssembly("DamaCoreCMS.Framework"));
                        break;
