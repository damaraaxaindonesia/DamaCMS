...
using DamaCoreCMS.Framework.Core.Extensions;
using DamaCoreCMS.Framework.Resources;
using DamaCoreCMS.Framework.Core;
using System.Linq;
using DamaCoreCMS.Framework.Resources;
...
namespace DamaCoreCMS.Framework.Setup
{
    public class SetupHelper
    {
        ...
        private static NccPermission CreatePermissionObject(string roleName)
        {
            ...
            using (Stream resource = assembly.GetManifestResourceStream($"DamaCoreCMS.Framework.Resources.DefaultPermissions.{roleName}.json"))
            {
        ...
        public static async Task SaveBasicData(NccUser nccUser, NccDbContext nccDbConetxt, UserManager<NccUser> userManager, RoleManager<NccRole> roleManager, NccSignInManager<NccUser> signInManager, WebSiteInfo setupInfo)
        {
            ...
            //    WidgetConfigJson = "",
            //    ModuleId = "DamaCoreCMS.Core.Modules.Cms",
            //    WidgetId = "DamaCoreCMS.Modules.Cms.CmsSearch",
            //    ThemeId = "com.DamaCoreCMS.themes.NccSeventeen",
            ...
            switch (dbe)
            {
                case SupportedDatabases.MSSQL:
                    GlobalContext.Services.AddDbContext<NccDbContext>(options =>
                        options.UseSqlServer(SetupHelper.ConnectionString, opts => opts.MigrationsAssembly("DamaCoreCMS.Framework"))
                    );                    
                    break;
                ...
                case SupportedDatabases.MySql:
                    GlobalContext.Services.AddDbContext<NccDbContext>(options =>
                        options.UseMySql(SetupHelper.ConnectionString, opts => opts.MigrationsAssembly("DamaCoreCMS.Framework"))
                    );
                    
                    break;
                case SupportedDatabases.SqLite:
                    GlobalContext.Services.AddDbContext<NccDbContext>(options =>
                        options.UseSqlite(SetupHelper.ConnectionString, opts => opts.MigrationsAssembly("DamaCoreCMS.Framework"))
                    );
